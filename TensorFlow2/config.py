...
def test_parser():
	...
	parser.add_argument('-p', '--path', metavar = 'P', type = str,
						# required = True,
						default="Weights/VRP20_train_epoch19.h5",
						help = 'Weights/VRP***_train_epoch***.h5, h5 file required')
	parser.add_argument('-b', '--batch', metavar = 'B', type = int, default = 1, help = 'batch size')
	parser.add_argument('-n', '--n_customer', metavar = 'N', type = int, default = 15, help = 'number of customer nodes, time sequence')
	...
	parser.add_argument('-d', '--decode_type', metavar = 'D', type = str,
						# required = True,
						default="sampling",
						choices = ['greedy', 'sampling'], help = 'greedy or sampling required')
	args = parser.parse_args()
	return args
